...
class UserApiOmv(models.Model):
    ...
    codigo_omv = models.ForeignKey(
        Omv, verbose_name=("Codigo OMV"), related_name='UserOmv_Omv',on_delete=models.CASCADE)
...
class CallApi(models.Model):
    ...
    call_id = models.IntegerField(
        primary_key=True, verbose_name=("ID"), editable=False)
    codigo_omv = models.ForeignKey(
        Omv, verbose_name=("Codigo OMV"), related_name='UserOmv_CallApi',on_delete=models.CASCADE)
    typecall = models.ForeignKey(
        TypeCallApi, verbose_name=("Tipo de Call"), related_name='TypeCallApi_CallApi',on_delete=models.CASCADE)
...
class ParamCallApi(models.Model):
    ...
    call_id = models.ForeignKey(
        CallApi, verbose_name=("Call OMV"), related_name='CallApi_ParamCallApi',on_delete=models.CASCADE)
...
class ParamCallBackApi(models.Model):
    id = models.IntegerField(
        primary_key=True, verbose_name=("ID"), editable=False)
    call_id = models.ForeignKey(
        CallApi, verbose_name=("Call OMV"), related_name='CallApi_ParamCallBackApi',on_delete=models.CASCADE)
    param_name = models.CharField(
        verbose_name=("param nombre"), max_length=120)
